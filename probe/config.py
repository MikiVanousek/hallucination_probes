...
class ProbeConfig:
    ...
    hf_repo_id: Optional[str] = (
        "andyrdt/hallucination-probes"  # HuggingFace repository ID
    )
    ...
    def __post_init__(self):
        ...
        if self.load_from in ["disk"] and not self.probe_path.exists():
            raise ValueError(
                f"Probe with ID {self.probe_id} not found in disk at path {self.probe_path}"
            )
        ...
        if isinstance(self.lora_layers, str):
            if self.lora_layers == "all":
                ...
                self.lora_layers = [
                    int(layer)
                    for layer in self.lora_layers.strip("[").strip("]").split(",")
                ]
...
class TrainingConfig:
    ...
    high_loss_threshold: Optional[float] = (
        None  # Threshold for masking high-loss tokens
    )
    ...
    train_dataset_configs: List[TokenizedProbingDatasetConfig] = field(
        default_factory=list, init=False
    )
    eval_dataset_configs: List[TokenizedProbingDatasetConfig] = field(
        default_factory=list, init=False
    )
    ...
    def __post_init__(self):
        ...
        self.train_dataset_configs = [
            TokenizedProbingDatasetConfig(**config) for config in self.train_datasets
        ]

        self.eval_dataset_configs = [
            TokenizedProbingDatasetConfig(**config) for config in self.eval_datasets
        ]
