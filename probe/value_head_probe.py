...
class ValueHeadProbe(nn.Module):
    ...
    def __init__(
        ...
        assert layer_idx or path, (
            "Either path or layer index must be provided, otherwise we can't infer the layer where we should hook the value head to."
        )
        ...
        if path:
            ...
            self.value_head = nn.Linear(
                hidden_size, 1, device=model.device, dtype=model.dtype
            )
    ...
    def save(self, path: Union[str, Path]):
        ...
        torch.save(self.value_head.state_dict(), path / "probe_head.bin")
    ...
    @classmethod
    def load_head(
        cls, path: Path, device: str = "cuda", dtype: torch.dtype = torch.bfloat16
    ) -> Tuple[nn.Module, int]:
        """
        ...
        state_dict = torch.load(
            path / "probe_head.bin", map_location=device, weights_only=True
        )
...
def setup_probe(
    model: PreTrainedModel, probe_config: ProbeConfig
    ...
    if probe_config.load_from == "hf" and not probe_config.probe_path.exists():
        download_probe_from_hf(
            repo_id=probe_config.hf_repo_id, probe_id=probe_config.probe_id
        )
    ...
    if probe_config.load_from in ["hf", "disk"]:
        ...
        if probe_config.lora_layers:
            print(
                f"Initializing new LoRA adapters for layers {probe_config.lora_layers}"
            )
            model = setup_lora_for_layers(
                ...
                lora_module_suffixes=probe_config.lora_module_suffixes,
            )
